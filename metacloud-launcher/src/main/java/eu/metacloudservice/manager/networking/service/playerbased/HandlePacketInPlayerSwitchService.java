    ...
    public void handle(Channel channel, Packet packet) {
        if (packet instanceof PacketInPlayerSwitchService){
            if (!CloudManager.shutdown){
                CloudPlayerRestCech restCech = (CloudPlayerRestCech)(new RestDriver()).convert(Driver.getInstance().getWebServer().getRoute("/cloudplayer/" + UUIDDriver.getUUID(((PacketInPlayerSwitchService) packet).getName())), CloudPlayerRestCech.class);
                ManagerConfig config = (ManagerConfig)(new ConfigDriver("./service.json")).read(ManagerConfig.class);
                if (config.isShowConnectingPlayers()){
                    Driver.getInstance().getTerminalDriver().logSpeed(Type.NETWORK, "Der Spieler '"+ ((PacketInPlayerSwitchService) packet).getName() + "@" + restCech.getUuid() + "hat sich mit dem Server '"+ ((PacketInPlayerSwitchService) packet).getServer() + "§r' verbunden",
                            "The player '"+ ((PacketInPlayerSwitchService) packet).getName()+ "@" + restCech.getUuid() + "has connected to the '"+ ((PacketInPlayerSwitchService) packet).getServer()+ "§r' server");
                }
