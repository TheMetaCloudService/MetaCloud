    ...
    public void handle(Channel channel, Packet packet) {
        if (packet instanceof PacketOutPlayerSwitchService){
            ...
            if (!CloudAPI.getInstance().getPlayerPool().playerIsNotNull(((PacketOutPlayerSwitchService) packet).getName())){
                AsyncCloudAPI.getInstance().getPlayerPool().registerPlayer(new eu.metacloudservice.async.pool.player.entrys.CloudPlayer(((PacketOutPlayerSwitchService) packet).getName(), UUIDDriver.getUUID(((PacketOutPlayerSwitchService) packet).getName())));
