...
import eu.metacloudservice.networking.out.node.PacketOutNodeConnect;
import eu.metacloudservice.networking.out.node.PacketOutNodeCreate;
import eu.metacloudservice.networking.out.node.PacketOutNodeDelete;
import eu.metacloudservice.networking.out.node.PacketOutNodeDisconnect;
...
>>>>>>> 76720eda39736bb3c09717d884b4a312695eb6da
import eu.metacloudservice.networking.packet.Packet;
import eu.metacloudservice.pool.groupe.GroupPool;
import eu.metacloudservice.pool.node.NodePool;
...
public class CloudAPI {
    ...
    public CloudAPI(boolean isVelo) {
        instance = this;
        new Driver();
        Driver.getInstance().getMessageStorage().authKey =  ((AuthenticatorKey) new ConfigDriver("./connection.key").read(AuthenticatorKey.class)).getKey();
        ...
        NettyDriver.getInstance().getPacketDriver()
                .registerHandler(new PacketOutNodeConnect().getPacketUUID(), new HandlePacketOutNodeConnect(), PacketOutNodeConnect.class)
                .registerHandler(new PacketOutNodeDelete().getPacketUUID(), new HandlePacketOutNodeDelete(), PacketOutNodeDelete.class)
                .registerHandler(new PacketOutNodeCreate().getPacketUUID(), new HandlePacketOutNodeCreate(), PacketOutNodeCreate.class)
                .registerHandler(new PacketOutNodeDisconnect().getPacketUUID(), new HandlePacketOutNodeDisconnect(), PacketOutNodeDisconnect.class)
